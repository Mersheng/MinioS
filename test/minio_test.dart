...
void testFPutObject() {
  group('fPutObject', () {
    ...
    test('fPutObject() inserts content-type to metadata', () async {
      final minio = getMinioClient();
      await minio.fPutObject(bucketName, objectName, testFile.path, '<token>');
    ...
    test('fPutObject() adds user-defined object metadata w/ prefix', () async {
      ...
      await minio.fPutObject(
          bucketName, objectName, testFile.path, 'token', metadata);
    ...
    test('fPutObject() with empty file', () async {
      ...
      final minio = getMinioClient();
      await minio.fPutObject(bucketName, objectName, emptyFile.path, 'token');
